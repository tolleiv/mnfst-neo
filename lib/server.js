...
var _showServer =[
    'START n=node({id}) MATCH (n:Server)',
    'OPTIONAL MATCH (n:Server)-[r:USES]-(f:File)--(rr:Resource)-[rrr:CHANGES]-(n)',
    'RETURN  {server:n, resources:collect(DISTINCT [rr,rrr]), files: collect(DISTINCT f)}'
];
...
module.exports = {
    showServer: query(_showServer).post(_showData).run,
    show: query(_show).post(_showSingleServers).run,
    list: query(_listAll).post(_showAllServers).run,
    getServer: query(_findServer).run,
    linkServerToFile: query(_linkServerToFile).post(_showData).run,
    unlinkServerFromFiles: query(_unlinkServerFromFiles).run,
    updateServerProperties: query(_updateServerProperties).post(_showData).run,
