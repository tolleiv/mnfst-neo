...
var _listAll = function(callback) {
    var query = [
        ...
        'OPTIONAL MATCH (s)-[rr:CHANGES]-()',
        'RETURN s, AVG(rr.rate10) AS rate, COUNT(rr) AS resource_count, COUNT(f) AS file_count',
        'ORDER BY rate DESC'
    ];
...
var _updateServerResourceRate = function(params, callback) {
    var cypher_params = {
        fqdn: params.fqdn,
        resource: params.changes
    };
    var query = [
        'MATCH (s:Server)<-[rr:CHANGES]-(r:Resource)',
        'WHERE s.fqdn = {fqdn}',
        'SET rr.rate10 = CASE',
        'WHEN r.name IN {resource} THEN rr.rate10*0.9 + 0.1',
        'ELSE rr.rate10*0.9',
        'END'
    ];
